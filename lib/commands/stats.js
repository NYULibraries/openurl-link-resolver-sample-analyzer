...
const ERRORS = {
    ARIADNE: {
        BACKEND_API_FETCH_ERROR_FAILED_TO_FETCH: 'Error fetching data from the Ariadne API: TypeError: Failed to fetch',
...
function getStatsJson( linksReportFile ) {
    const linksJson = require( linksReportFile );
    ...
    Object.keys( linksJson ).sort().forEach( testCaseUrl => {
        ...
        // # links: All Total
        const numLinksAllTotal = numLinksCommon + numLinksAriadneOnly + numLinksGetItOnly;
